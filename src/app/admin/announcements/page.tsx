...
export default function AdminAnnouncementsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <PageHeader
        title="Announcements"
        subtitle="Send a scrolling marquee announcement to CRs, Staff, or other Admins on your campus."
        action={
          <button
      ...
      <AnnouncementsManager refreshKey={refreshKey} />
...
function AnnouncementsManager({ refreshKey }: { refreshKey: number }) {
  ...
  async function handleDelete(a: Announcement) {
    const ok = await confirmAction(
      `"${a.title}" will be removed from every recipient's notifications immediately. This can't be undone.`,
      ...
      <div className="w-full max-w-lg rounded-xl bg-white p-5 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <textarea
            ...
            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-accent-500 focus:outline-none"
          />
          <p className="text-xs text-slate-400">This updates the announcement for everyone who already received it.</p>
...
function NewAnnouncementModal({ onClose, onSent }: { onClose: () => void; onSent: () => void }) {
  ...
  const [audience, setAudience] = useState<"cr" | "staff" | "admin">("cr");
  ...
  const audienceLabel = { admin: "admin", staff: "Staff", cr: "CR" }[audience];
        ...
        {sentTo !== null ? (
          <div className="py-4 text-center">
            <p className="text-sm text-slate-700">
              Announcement sent to {sentTo} {audienceLabel}{sentTo === 1 ? "" : audience === "staff" ? "" : "s"}.
            </p>
            <button
              onClick={onSent}
            ...
            <div className="rounded-lg border border-slate-200 p-3">
              ...
              <div className="mb-3 grid grid-cols-3 gap-2">
                ...
                <button
                  ...
                  className={`rounded-lg border px-3 py-2 text-sm font-medium ${
                    audience === "staff" ? "border-accent-500 bg-accent-50 text-accent-700" : "border-slate-300 text-slate-600 hover:bg-slate-50"
                  }`}
                >
                  Staff
                </button>
                ...
                <>
                  {isSuperAdmin ? (
                    ...
                    <p className="text-xs text-slate-500">
                      Goes to Staff on your campus{user?.campus ? ` (${campusLabel(user.campus, campuses)})` : ""}.
                    </p>
                  )}
                </>
              ) : (
